CREATE TABLE stg_ebitda_long AS
WITH base AS (
    SELECT
        ROWID AS row_id_original,
        TRIM(EBITDA_RECONCILIATION_THOUSAND_OF_R) AS conta_en,
        TRIM(RECONCILIAÇÃO_DO_EBITDA_R_MIL) AS conta_pt,
    ...
    FROM raw_ebitda
    WHERE
        EBITDA_RECONCILIATION_THOUSAND_OF_R IS NOT NULL
        AND TRIM(EBITDA_RECONCILIATION_THOUSAND_OF_R) <> ''
),
